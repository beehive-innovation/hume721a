...
async function main() {
  ...
  const hume721A: Hume721A = (await ethers.getContractAt(
    (
  ...
  assert(
    (await hume721A.ownerOf(1)) == signers[0].address,
    "before Wrong Owner"
  );
  assert(
    (await hume721A.ownerOf(25)) == signers[0].address,
    "before Wrong Owner"
  );
  assert(
    (await hume721A.ownerOf(50)) == signers[0].address,
    "before Wrong Owner"
  );

  for (let i = 0; i < addresses.length; i++) {
    let tx = await hume721A.transferFrom(
      signers[0].address,
      addresses[i],
      i + 1
    );
