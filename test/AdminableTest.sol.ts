...
before(async () => {
    ...
    adminableContract = await AdminableContract.connect(admin).deploy(admin.address) as AdminableTest;

    // Deploying the Adminable contract
    const AdminableContract2 = await ethers.getContractFactory("AdminableTest");
    adminableContract2 = await AdminableContract2.connect(admin).deploy(admin.address) as AdminableTest;
})
...
describe("Adminable Functionality Test", () => {
    ...
    it("Should emit AdminTransferred event", async () => {
        expect(await adminableContract.transferAdmin(new_Admin.address)).to.emit(adminableContract, "AdminTransferred");
    });

    // new_Admin --> admin
    it("Should transfer admin", async () => {

        let adminTransaction = await adminableContract.connect(new_Admin).transferAdmin(admin.address);

        const { previousAdmin, newAdmin } = await getEventArgs(adminTransaction, "AdminTransferred", adminableContract);
    ...
    it("Non Admin user should not be able to transfer admin", async () => {
        await expect(adminableContract.connect(new_Admin).transferAdmin(admin.address)).to.revertedWith("Adminable: caller is not the admin");
    });

    it("Should not be able to transfer adminShip to zero-address", async () => {
        await expect(adminableContract.transferAdmin(ethers.constants.AddressZero)).to.revertedWith("Adminable: new admin is the zero address");
    });
